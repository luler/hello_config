...
class JwtTools
{
     ...
     */
    public function getClientInfo($token = '')
    {
        if (empty($token)) {
            $token = self::getCurrentToken();
        }
        if (preg_match('/' . self::$prefix . '\s*(.*)\b/i', $token, $matches)) {
            $jwt_token = $matches[1];
        } else {
     ...
     * @throws UnauthorizedHttpException
     */
    public function verification($jwt_token)
    {
        //检验是否为注销token
        if (Cache::has('logout_token:' . $jwt_token)) {
            throw new UnauthorizedHttpException();
        }
        $key = $this->getKey();
        try {
            $decoded = (array)JWT::decode($jwt_token, $key, ['HS256']); //HS256方式，这里要和签发的时候对应
    ...
    public function logout($token = '')
    {
        //加入注销缓存
        if (!empty($token)) {
            $jwt_token = $this->getClientInfo($token);
        }
        Cache::set('logout_token:' . $jwt_token, $jwt_token, $this->getRefreshTimeOut());
    }
}
